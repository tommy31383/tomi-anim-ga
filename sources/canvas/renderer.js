...
/**
 * Bake the "Tưng tưng" CSS hop into a real horizontal PNG sheet.
 *
 * Picks the south-facing walk frame 0 (standing pose), then renders N
 * transformed copies along a row using the same squash/stretch numbers as
 * the live CSS keyframe (@keyframes tomi-hop). Each tile is taller than
 * the source frame so the sprite has room to lift.
 *
 * @param {number} numFrames how many frames to bake (default 8)
 * @returns {HTMLCanvasElement|null}
 */
export function bakeTungTungSheet(numFrames = 8) {
  if (!canvas) return null;

  // Walk row 0 lives at ANIMATION_OFFSETS.walk; south = row 2.
  const walkBaseY = ANIMATION_OFFSETS.walk;
  const SOUTH_ROW = 2;
  const srcY = walkBaseY + SOUTH_ROW * FRAME_SIZE;
  const srcX = 0; // frame 0 = standing

  // Reject if region is empty (no body selected).
  const srcCtx = get2DContext(canvas, true);
  let hasPixel = false;
  outer: for (let dy = 0; dy < FRAME_SIZE; dy += 4) {
    for (let dx = 0; dx < FRAME_SIZE; dx += 4) {
      if (srcCtx.getImageData(srcX + dx, srcY + dy, 1, 1).data[3] > 0) {
        hasPixel = true;
        break outer;
      }
    }
  ...
  // Extract source sprite into its own canvas for transform-friendly draws.
  const sprite = document.createElement("canvas");
  sprite.width = FRAME_SIZE;
  sprite.height = FRAME_SIZE;
  ...
  // Tile = source frame size + headroom for vertical hop (22px) + a bit.
  const TILE = FRAME_SIZE + 32; // 96px tall/wide per frame
  const out = document.createElement("canvas");
  out.width = numFrames * TILE;
  out.height = TILE;
  const ctx = get2DContext(out);
  ctx.imageSmoothingEnabled = false;

  // Match the @keyframes tomi-hop curve:
  //   0% / 100%: scaleX 1.18, scaleY 0.78, translateY 0   (squash on ground)
  //   50%:      scaleX 0.92, scaleY 1.14, translateY -22  (stretched apex)
  // Phase eases via ½(1 - cos(t·2π)) so 0→1→0 across the cycle (smooth).
  const HOP_HEIGHT = 22;
  const SCALE_X_GROUND = 1.18;
  const SCALE_X_APEX = 0.92;
  const SCALE_Y_GROUND = 0.78;
  const SCALE_Y_APEX = 1.14;

  for (let i = 0; i < numFrames; i++) {
    const t = i / numFrames; // 0 → 1
    const phase = 0.5 - 0.5 * Math.cos(t * Math.PI * 2); // 0 → 1 → 0
    const sx = SCALE_X_GROUND + phase * (SCALE_X_APEX - SCALE_X_GROUND);
    const sy = SCALE_Y_GROUND + phase * (SCALE_Y_APEX - SCALE_Y_GROUND);
    const ty = -HOP_HEIGHT * phase;

    // Anchor: bottom-center of tile = where the feet plant when grounded.
    const cx = i * TILE + TILE / 2;
    const cy = TILE; // bottom edge of the tile

    ctx.save();
    ctx.translate(cx, cy + ty);
    ctx.scale(sx, sy);
    // Draw sprite anchored at its bottom-center.
    ctx.drawImage(sprite, -FRAME_SIZE / 2, -FRAME_SIZE);
    ctx.restore();
  }
